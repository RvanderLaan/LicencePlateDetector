...
% Last Modified by GUIDE v2.5 17-Dec-2014 09:30:03
...
% Get start time in ms (to play at 30 fps)
tic;

for i = 2:frames
    axes(handles.axes1);
    % Store when this cycle starts
    time = toc;
    ...
    % Calculate time and pause to play at desired frame rate
    delta = toc - time;
    if delta < 1/rate
        % Keep looping until time is right (because pause() was very
        % inaccurate)
        while (toc < time + 1/rate) 
        end;
    end;
end;
